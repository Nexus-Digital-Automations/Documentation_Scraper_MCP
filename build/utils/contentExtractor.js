...
export class ContentExtractor {
     ...
     */
    async removeExcludedElements(page, url) {
        try {
            ...
            if (selectorsToExclude && selectorsToExclude.length > 0) {
                this.logger.info(`Attempting to remove ${selectorsToExclude.length} patterns of excluded elements from DOM`, { url, selectors: selectorsToExclude });
                try {
                    ...
                    this.logger.debug('Successfully executed DOM removal script for excluded selectors', { url });
                }
                catch (evalError) {
                    // This error occurs if page.evaluate itself fails
                    this.logger.error('Error during page.evaluate for removing selectorsToExcludeFromText', { url, error: getErrorMessage(evalError) });
                    // Depending on severity, you might choose to return or continue
                }
            }
            else {
                this.logger.debug('No selectorsToExcludeFromText configured or list is empty', { url });
            }
        }
        catch (error) {
            const errorMessage = getErrorMessage(error);
            this.logger.error(`Element exclusion failed: ${errorMessage}`, { url });
