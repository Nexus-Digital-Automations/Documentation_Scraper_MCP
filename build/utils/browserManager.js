...
export class BrowserManager {
    ...
    async launchBrowser() {
        try {
            ...
            this.logger.log(`Browser launched successfully with stealth configuration`, '/tmp/browser_manager.log', { logLevel: 'SUCCESS' });
            return browser;
        }
        catch (error) {
            const errorMessage = getErrorMessage(error);
            this.logger.log(`Browser launch failed: ${errorMessage}`, '/tmp/browser_manager.log', { logLevel: 'ERROR' });
    ...
    async createPage(browser) {
        try {
            ...
            this.logger.log(`Page created successfully with default configuration`, '/tmp/browser_manager.log', { logLevel: 'DEBUG' });
            return page;
        }
        catch (error) {
            const errorMessage = getErrorMessage(error);
            this.logger.log(`Page creation failed: ${errorMessage}`, '/tmp/browser_manager.log', { logLevel: 'ERROR' });
     ...
     */
    async configurePageDefaults(page) {
        try {
            ...
            this.logger.log(`Page configuration failed: ${getErrorMessage(error)}`, '/tmp/browser_manager.log', { logLevel: 'WARN' });
        }
    }
    /**
    ...
    async closePage(page) {
        try {
            if (page && !page.isClosed()) {
                ...
                this.logger.log(`Page closed successfully`, '/tmp/browser_manager.log', { logLevel: 'DEBUG' });
            }
        }
        catch (error) {
            this.logger.log(`Page closure failed: ${getErrorMessage(error)}`, '/tmp/browser_manager.log', { logLevel: 'WARN' });
        }
    }
    /**
    ...
    async closeBrowser(browser) {
        try {
            if (browser && browser.connected) {
                ...
                this.logger.log(`Browser closed successfully`, '/tmp/browser_manager.log', { logLevel: 'SUCCESS' });
            }
        }
        catch (error) {
            this.logger.log(`Browser closure failed: ${getErrorMessage(error)}`, '/tmp/browser_manager.log', { logLevel: 'WARN' });
        }
    }
    /**
     ...
     */
    async handleBrowserMemoryCleanup() {
        try {
            const memoryUsage = process.memoryUsage();
            const heapUsedMB = Math.round(memoryUsage.heapUsed / 1024 / 1024);
            if (heapUsedMB > this.config.resourceMonitoring.memoryThresholdMB) {
                this.logger.log(`Memory threshold exceeded (${heapUsedMB}MB), initiating cleanup`, '/tmp/browser_manager.log', { logLevel: 'WARN' });
            ...
            this.logger.log(`Memory cleanup failed: ${getErrorMessage(error)}`, '/tmp/browser_manager.log', { logLevel: 'ERROR' });
        }
    }
    /**
    ...
    async cleanup() {
        try {
            ...
            this.logger.log(`BrowserManager cleanup completed successfully`, '/tmp/browser_manager.log', { logLevel: 'SUCCESS' });
        }
        catch (error) {
            this.logger.log(`BrowserManager cleanup failed: ${getErrorMessage(error)}`, '/tmp/browser_manager.log', { logLevel: 'ERROR' });
        }
    }
}
