...
import { ChangeDetectionMonitor } from '../utils/changeDetectionMonitor.js';
...
export class UrlDiscoveryEngine {
     ...
     */
    async discoverUrls(args, context) {
        ...
        try {
            ...
            const crawlQueue = [{ url: normalizedStartUrl, depth: 0 }];
            ...
            while (crawlQueue.length > 0) {
                ...
                const batchPromises = currentBatch.map(item => limit(async () => {
                    try {
     ...
     */
    async processUrl(item, args, outputDir) {
        let browser = null;
        let page = null;
        ...
        try {
            ...
            if (this.structuredDataExtractor && this.config.contentExtraction?.structuredDataSchemas) {
                try {
                    const structuredData = await this.structuredDataExtractor.extractStructuredData(page, item.url, this.config.contentExtraction.structuredDataSchemas);
     ...
     */
    async clickToggleButtons(page, sessionTextBasedClickTargets) {
                ...
                try {
                    ...
                    for (const targetElement of clickTargetElements) {
                        try {
                            ...
                            if (textSourceElement) {
                                textToCheck = await textSourceElement.evaluate((el) => el.innerText || '');
                            }
                            if (!textToCheck.trim()) {
     ...
     */
    async clickAndVerify(page, element, identifier) {
        ...
        try {
            ...
            await page.waitForFunction((el) => {
                ...
                if (el.classList.contains('active') || el.classList.contains('open') || el.classList.contains('expanded'))
                    return true;
                ...
                return false;
            }, { timeout: this.config.contentExtraction.toggleExpansionTimeout || 5000 }, element);
     ...
     */
    shouldIncludeUrl(url, keywords, excludePatterns) {
        try {
            ...
            if (keywords && keywords.length > 0) {
                ...
                const validKeywords = keywords.filter(keyword => keyword && typeof keyword === 'string' && keyword.trim().length > 0);
