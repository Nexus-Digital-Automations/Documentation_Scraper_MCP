...
export class StructuredDataExtractor {
  ...
  private async extractUsingSchema(page: Page, schema: StructuredDataSchema, url: string): Promise<ExtractedStructuredData | null> {
    try {
      this.validationErrors = [];
  ...
  private async generateOutputFiles(extractionResult: ExtractedStructuredData, schema: StructuredDataSchema): Promise<string[]> {
    try {
