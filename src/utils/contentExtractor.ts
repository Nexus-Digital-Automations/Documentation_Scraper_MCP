...
export class ContentExtractor {
  ...
  private outputDirectories: Set<string>;
  ...
  async extractAndSaveText(
    ...
    try {
      ...
      if (extractedContent.length < 50) {
        this.logger.log(
          `Warning: Extracted content is very short (${extractedContent.length} chars)`,
          '/tmp/content_extractor.log',
          { logLevel: 'WARN' }
        );
      }
      ...
      this.logger.log(
        `Text content extracted and saved successfully`,
        '/tmp/content_extractor.log',
        { logLevel: 'SUCCESS' }
      );
   ...
   */
  private async extractUsingSelectors(page: Page, selectors: string[]): Promise<string> {
    try {
      ...
      for (const selector of selectors) {
        try {
          ...
          this.logger.log(
            `Selector failed: ${selector} - ${getErrorMessage(selectorError)}`,
            '/tmp/content_extractor.log',
            { logLevel: 'DEBUG' }
          );
        }
      ...
      if (!extractedContent || extractedContent.length < 100) {
        const bodyContent = await page.evaluate(() => {
          return document.body?.textContent || (document.body as HTMLElement)?.innerText || '';
        });
   ...
   */
  async organizeOutputFiles(): Promise<void> {
    try {
      ...
      this.logger.log(
        `Output directory structure organized successfully`,
        '/tmp/content_extractor.log',
        { logLevel: 'SUCCESS' }
      );
    } catch (error) {
      const errorMessage = getErrorMessage(error);
      this.logger.log(
        `Output organization failed: ${errorMessage}`,
        '/tmp/content_extractor.log',
        { logLevel: 'ERROR' }
      );
   ...
   */
  private generateUniqueFilename(url: string, extension: string): string {
    try {
   ...
   */
  private extractHostnameForFilename(url: string): string {
    try {
