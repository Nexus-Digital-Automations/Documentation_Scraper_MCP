...
import { InteractionStep, PageInteractionConfig } from '../config.js';
...
export class InteractionHandler {
   ...
   */
  async executeLoadWaitConditions(page: Page): Promise<boolean> {
    try {
   ...
   * @returns Promise<boolean> - True if all conditions satisfied successfully
   */
  async executePostClickWaitConditions(page: Page): Promise<boolean> {
    try {
   ...
   */
  async executeCustomSequence(page: Page, sequenceName: string): Promise<boolean> {
    try {
   ...
   */
  private async executeInteractionStep(page: Page, step: InteractionStep, contextId: string): Promise<boolean> {
    try {
   ...
   */
  private async executeScrollAction(page: Page, direction: string, distance: number): Promise<void> {
    try {
      ...
      await page.evaluate((dir: string, dist: number) => {
        let x = 0, y = 0;
   ...
   */
  private isJavaScriptFunctionSafe(functionCode: string): boolean {
    try {
